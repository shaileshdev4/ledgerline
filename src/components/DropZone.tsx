...
const FILE_TYPES: { icon: IconType; label: string; hint: string }[] = [
  ...
  { icon: HiDocumentText, label: "Receipts", hint: "photos or receipts.json" },
  {
    ...
    label: "P2P screenshots",
    hint: "Venmo / Zelle images or JSON",
  },
  {
    icon: HiTableCells,
    label: "Ledger sheet",
    hint: "ledger.csv or spreadsheet export",
  },
];
...
export function DropZone({
  ...
  const loadSampleBooks = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Could not load sample books",
      );
      setStatus(null);
      setIngesting(false);
    }
    ...
    >
      <div style={{ width: "100%", maxWidth: 640 }}>
        ...
        >
          <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <input
              ...
              disabled={busy}
              style={inputStyle}
              placeholder="e.g. Youth Code Foundation"
            />
          </label>
          <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        >
          ...
          >
            Receipt photos, bank CSV, P2P screenshots, ledger spreadsheet -
            mixed folders welcome. Images are OCR&apos;d in-browser via Puter.
          </div>
